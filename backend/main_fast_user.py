...
class EnhancedUserBot:
    ...
    def run_continuous_applications(self, max_applications=None):
        ...
        try:
            ...
            def enhanced_write_to_file(company, job_title, link, location, search_location):
                ...
                self.last_application_time = datetime.now()
                logger.info(f"⏰ Updated last application time: {self.last_application_time}")
            ...
            self.last_application_time = datetime.now()
            logger.info(f"⏰ Timeout tracking initialized - will restart if no application in {self.timeout_minutes} minutes")
            ...
            while not self.stop_requested:
                try:
                    ...
                    if self.check_timeout_and_restart():
                        logger.info("🔄 Bot restarted due to timeout, continuing...")
                        continue
    ...
    def notify_bot_manager(self, job_data):
        ...
        try:
            # Try to import and use the enhanced bot manager
            sys.path.append(os.path.join(os.path.dirname(__file__), 'backend'))
            
            # Try to get the enhanced bot manager instance
            try:
                from enhanced_bot_manager import enhanced_bot_manager
                if enhanced_bot_manager:
                    enhanced_bot_manager.log_application(self.user_id, job_data)
                    logger.info(f"🎯 ✅ Successfully notified enhanced bot manager about application")
                else:
                    logger.warning(f"🎯 ⚠️ Enhanced bot manager not available")
            except ImportError:
                logger.warning(f"🎯 ⚠️ Could not import enhanced bot manager")
            except Exception as e:
                logger.warning(f"🎯 ⚠️ Error notifying bot manager: {e}")
                
        except Exception as e:
            logger.warning(f"🎯 ⚠️ Failed to notify bot manager: {e}")
    ...
    def check_timeout_and_restart(self):
        ...
        if time_since_last_application.total_seconds() > timeout_seconds:
            if self.restart_count < self.max_restarts:
                logger.warning(f"⏰ TIMEOUT DETECTED: No application in {self.timeout_minutes} minutes")
                logger.warning(f"🔄 Auto-restarting bot (attempt {self.restart_count + 1}/{self.max_restarts})")
                ...
                # Reinitialize bot (recreate the entire bot session)
                logger.info("🔄 Reinitializing bot after timeout...")
                self.cleanup()
                
                # Reinitialize browser
                self.browser = self.init_browser_with_retry()
                if not self.browser:
                    raise Exception("Failed to reinitialize browser after timeout")
                
                # Recreate bot with same config
                self.bot = LinkedinEasyApply(self.config, self.browser)
                self.bot.fast_mode = False
                self.bot.continuous_mode = True
                
                # Re-setup the enhanced write_to_file method
                # We need to recreate the enhanced_write_to_file function for the new bot instance
                original_write_to_file = self.bot.write_to_file
                
                def enhanced_write_to_file_restart(company, job_title, link, location, search_location):
                    """Enhanced write_to_file that saves to database for dashboard updates"""
                    # Update last application time for timeout tracking
                    self.last_application_time = datetime.now()
                    logger.info(f"⏰ Updated last application time: {self.last_application_time}")
                ...
                self.bot.write_to_file = enhanced_write_to_file_restart
                ...
                logger.error(f"❌ MAX RESTARTS REACHED: {self.max_restarts} restarts attempted")
